...
public:
	...
	static const std::wstring	DELIM_QUERY;			///< query delimiting character '?'
	static const std::wstring	DELIM_QUERY_ITEM;		///< query item delimiting character '&'
	static const std::wstring	DELIM_FRAGMENT;			///< fragment delimiting character '#'
...
public:
	 ...
	 * @param fullURI fullURI is expected to be a percent-encoded UTF8 string.
	 */
 ...
 * outerQuery and outerFragment are optional.
 */
PRTX_EXPORTS_API URIPtr createCompositeURI(
		const URIPtr&       nestedURI,
		const std::wstring& outerScheme,       ///< Must be non-empty, percent encoded and not end with a colon (:).
		const std::wstring& outerPath,         ///< Must be non-empty, percent encoded and start with a forward slash.
		const std::wstring& outerQuery = {},   ///< Optional, must be percent encoded.
		const std::wstring& outerFragment = {} ///< Optional, must be percent encoded.
);
