...
public:
	...
	enum EmbeddedResources {
		ER_NO, 				///< Decoder or the underlying format does not have support for embedded resources
		ER_CAN_HANDLE 		///< Format has embedded resources, and Decoder can handle it
	};
	...
	DecoderFactory(
			...
			EmbeddedResources       canHandleER = ER_NO			///< [in] ER_CAN_HANDLE if the associated decoder can handle embedded resources.
	);
