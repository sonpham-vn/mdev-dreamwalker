...
public:
	...
	/**
	 * @param[out] status  Optional pointer to return the status.
	 * @returns the content type this encoder can encode.
	 */
	virtual ContentType getType(Status* status = 0) const = 0;

	/**
	 * @param[out] status  Optional pointer to return the status.
	 * @returns the extensions of the corresponding encoder separated by colons (';')
	 */
	virtual const wchar_t* getExtensions(Status* status = 0) const = 0;

	/**
	 * @param[out] status  Optional pointer to return the status.
	 * @returns the merit of the encoder. In case two encoders provide overlapping functionality,
	 *          the one with the higher merit should be preferred.
	 */
