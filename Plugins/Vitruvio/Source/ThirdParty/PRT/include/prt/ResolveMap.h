...
public:
	 ...
	 * @param uri The percent-encded URI to the (embedded) asset or texture. See \ref AppNotes for URI creation.
	 * @param embeddedEntries If mode is EEM_INSPECT_AND_ADD (default), the uri is inspected if it points to a resource for
	 *                           which a Decoder with a ResolveMapProvider which can handle embedded resources is
	 *                           registered in the ExtensionManager. If so, and the resource is inspected and embedded resources are added as
	 *                           (key, uri) pairs too.
	 *
	 * @returns status.
	 */
